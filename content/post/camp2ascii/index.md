...
# The Ugly 
Campbell distributes two CLI tools, `tob32.exe` (deprecated) and `csidft_convert.exe`, for this purpose, but they're only included with LoggerNet, which costs about 600 USD. Upgrading an existing LoggerNet license is “only” 350 USD. While Campbell Scientific employs many smart, helpful people, and LoggerNet offers valuable utilities for networking with dataloggers, paying 600 bucks for software that is arguably necessary to process files from a datalogger and sensors that I already likely paid them over 10k for feels...somewhat offensive. 
